 ...
 */
public class JFileSystem implements java.io.Serializable, FileSystem {


  /**
   * serialVersionUID for JFileSystem.
   */
  ...
  private JFileSystem() {
    ...
    root = new JDirectory("", new JPath("/"), null);
  ...
  public String recursiveList(Directory dir, int counter, Boolean printPath) {
    ...
    if (dir.getName().equals("")) {
      ...
      contents += "\t".repeat(counter) + dir.getPath() + ":";
    } else {
      contents += "\t".repeat(counter) + dir.getName();
    }

    contents += "\n";

    for (File f : dir.getFile()) {
      contents += "\t".repeat(counter + 1) + f.getName() + "\n";
    }

    for (Directory d : dir.getChildren()) {
      contents += recursiveList(d, counter + 1, printPath);
    }
