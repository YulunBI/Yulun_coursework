...
class RBFRegression():
    ...
    def predict(self, X):
        ...
        Bx = np.hstack((np.ones((X.shape[0], 1)),self._rbf_2d(X,0)))
        for i in range(1, self.K):
            Bx = np.hstack((Bx, self._rbf_2d(X,i)))
        return Bx @ self.parameters
        # ====================================================
    ...
    def fit_with_l2_regularization(self, train_X, train_Y, l2_coef):
        ...
        pad_train_X = np.hstack((np.ones((train_Y.shape[0],1)),self._rbf_2d(train_X,0)))
        for i in range(1,self.K):
            pad_train_X = np.hstack((pad_train_X,self._rbf_2d(train_X,i)))
        XTX = np.transpose(pad_train_X) @ pad_train_X
        P1 = np.linalg.inv(XTX + l2_coef*I)
        P2 = np.transpose(pad_train_X) @ train_Y
